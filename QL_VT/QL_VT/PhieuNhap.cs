...
namespace QL_VT
{
    public partial class PhieuNhap : Form
    {
        ...
        void LoadDuGioiTinh()
        {
            ...
            da_HD.Fill(ds, "NhaCC");
            cbTenNhaCC.DataSource = ds.Tables["NhaCC"];
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Bạn muốn xóa?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                dataGridView1.ReadOnly = false;
                SqlDataAdapter da_PN = new SqlDataAdapter("select * from PhieuNhap", conn);
                DataRowView drv = (DataRowView)dataGridView1.CurrentRow.DataBoundItem;
                DataRow dtRow = drv.Row;
                SqlDataAdapter da_HD = new SqlDataAdapter("select * from ChiTietPhieuNhap where MaHDNhap='" + dtRow[0].ToString() + "'", conn);
        ...
        {
            SqlDataAdapter da_CTHD = new SqlDataAdapter("select * from PhieuNhap", conn);
        ...
        private void dataGridView1_MouseClick(object sender, MouseEventArgs e)
        {
            if (flag)
            {
                ...
                string MaNCC = row[1].ToString(), MaNV = row[2].ToString();
                SqlDataAdapter da_HD = new SqlDataAdapter("select * from NhaCC where MaNCC='" + MaNCC + "'", conn);
                DataTable ab = new DataTable();
                da_HD.Fill(ab);
